...
const CollaborativeMindMap: React.FC<CollaborativeMindMapProps> = ({ content, initialMindMap }) => {
  ...
  const addResearchNode = useMutation(({ storage }, parentId: string, title: string, content: string, type: 'quick' | 'deep') => {
    ...
    if (mindMap && root) {
      ...
      try {
        const updatedRoot = findAndAddToNode(root);
        if (updatedRoot) {
          mindMap.set('root', updatedRoot);
  ...
  const addNode = useMutation(({ storage }, parentId: string, text: string) => {
    ...
    if (mindMap && root) {
      ...
      try {
        const updatedRoot = addNodeToTree(root);
        if (updatedRoot) {
          mindMap.set('root', updatedRoot);
          
          // Force immediate node refresh like we do for research nodes
          setTimeout(() => {
            const newNodes = convertToSimpleNodes({ root: updatedRoot });
            console.log('Node added, forcing refresh with', newNodes.length, 'nodes');
            setNodes(newNodes);
          }, 50);
        }
      } catch (error) {
  ...
  return (
    <div className="w-full h-[900px] bg-gray-50 relative overflow-auto">
      <div className="absolute top-2 left-2 z-10 bg-white/90 backdrop-blur-sm rounded-lg px-3 py-2 text-xs text-gray-600 shadow-sm">
        💡 Double-click nodes to edit • Right-click to add children • Drag to move • Hover for research (Q=Quick, D=Deep)
        <br />
      ...
      <div className="relative w-[2400px] h-[1800px] p-8">
        ...
        <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ zIndex: 1 }}>
          ...
          {(() => {
            ...
            if (root) {
              ...
              if (rootNode) {
                createConnections(root, rootNode.position);
              }
            }
